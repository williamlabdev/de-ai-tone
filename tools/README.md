...
- R-024：模糊比較匹配（中文 `比較[^\s，。]{1,4}`，英文 `kind of/sort of/somewhat/a bit/relatively/rather`；英文語料尚未實測校準；同句有無跟／和／比等比較對象靠人判）。
...
R-023（畫面字寫成一句人會講的話）沒有機器標記階段，全部人審：判準本身可寫成規則（短標籤冒號、全形分號並列），但要掃的卡片與圖說文字寫在 storyboard 檔案的表格欄位裡，`review-ui.html` 只讀取貼上的 script.zh.md 旁白全文，從不載入 storyboard 檔案，沒有欄位可套用，故未在 `review-ui.html` 實作。
